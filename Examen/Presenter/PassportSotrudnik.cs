...
namespace Examen.Main
{
    public partial class PassportSotrudnik : MetroForm
    {
        ...
        async private Task LoadFormData()
        {
            ...
            services.AddTransient<IRepository<Models.Passport>, PassportRepository>();
            services.AddSingleton<Models.PP_KADNIKOV3Entities>();

            using (var serviceProvider = services.BuildServiceProvider()) // Создаем DI-контейнер
            {
                var userRepository = serviceProvider.GetRequiredService<IRepository<Models.Passport>>(); // Получаем экземпляр PassportRepository
