...
namespace Examen.Main
{
    public partial class Doljnost : MetroForm
    {
       ...
       async private void FormLoad()
        {
            DataTable dt = await Core.Database.ExecuteQuery($@"select * from doljnost where fk_kod_sotrudnik = {Core.Database.userId}");
        ...
        async private void SaveMethod()
        {
            ...
            if (dt.Rows.Count == 0)
            {
                await Core.Database.ExecuteQuery($@"INSERT INTO doljnost
                                                                         ([fk_kod_sotrudnik]
            ...
            {
                await Core.Database.ExecuteQuery($@"UPDATE doljnost
                                                                          SET 
